  ...
  for (const lang of ['zh', 'en']) {
    ...
    for (const num of [27, 28, 29]) {
      ...
      const found = await page.evaluate((n) => {
        const s = document.querySelector('#slide-' + String(n).padStart(2, '0'));
        return s ? {title: s.querySelector('h2')?.textContent || s.querySelector('.section-zh')?.textContent || '?', kic: s.querySelector('.section-zh')?.textContent} : null;
      }, num);
      console.log('lang=' + lang, 'slide-' + num, '=>', JSON.stringify(found));
